...
def getmax(row,col):
	return max(int(triangle[row][col])+int(triangle[row+1][col]), int(triangle[row][col])+int(triangle[row+1][col+1]))

i = 0
rows = len(triangle)
row = rows - 2

while i in range(0, rows-1):
	cols = len(triangle[row])
	newrow = []
	j = 0
	while j in range(0, cols):
		newrow.append(getmax(row,j))
		j = j + 1
	i = i + 1
	triangle[row]=newrow
	row = row - 1
