...
for j in range(0, 20):
	row = []
	for i in range(j * 60, j * 60 + 60, 3):
		row.append(int(digits[i:i+2]))
		i = i + 1
	grid[j]=row

#calculate the largest horizontal product
...
#calculate the largest vertical product
...
#calculate the largest left diagonal product
...
#calculate the largest right diagonal product
for x in range(0, 17):
	for y in range(3, 20):
		product = grid[x][y] * grid[x+1][y-1] * grid[x+2][y-2] * grid[x+3][y-3]
		if product > largest:
